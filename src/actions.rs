...
use sqlx_rxqlite::query_result::RXQLiteQueryResult as SqliteQueryResult;
pub type ConnectionPool = sqlx_rxqlite::RXQLitePool;
/*
#[cfg(feature = "postgres")]
...
pub async fn remove_filtered_policy(
    ...
    println!("{}({})remove_filtered_policy: pt: {},field_index: {},field_values: {:?}",file!(),line!(),pt,field_index,field_values);
    let field_values = normalize_casbin_rule_option(field_values);
    let boxed_query = if field_index == 5 {
        Box::new(sqlx::query(
            "DELETE FROM casbin_rule WHERE
...
//#[cfg(feature = "sqlite")]
pub(crate) async fn load_policy(conn: &ConnectionPool) -> Result<Vec<CasbinRule>> {
    let casbin_rule: Vec<CasbinRule> = sqlx::query_as("SELECT * FROM casbin_rule")
        .fetch_all(conn)
        .await
        .map_err(|err| CasbinError::from(AdapterError(Box::new(Error::SqlxError(err)))))?;
